...
#We must add IADDQ here in order to make additions faster
#So I added IADDQ just like in part B
#By that, I purpose to speed up summing a register with a number process faster.
#I am adding the same things I add on part B to make things easier.
#IADDQ adds an instant number to a register
...
#First we need to add IADDQ into the valid instructions
#Does fetched instruction require a regid byte? Yes, it does, so we add it to need_regids
#Does fetched instruction require a constant word? Yes, it does, so we add it to need_valC
# What register should be used as the B source? rB, so we add it to src - rB
# What register should be used as the E destination? rB, so we add it to dstE - rB
# Select input A to ALU. valC so we add it to valC
# Should the condition codes be updated? Yes, so it is added to set_cc
...
bool need_regids =
	f_icode in {IIADDQ, IRRMOVQ, IOPQ, IPUSHQ, IPOPQ, 
...
word aluB = [
	E_icode in { IRMMOVQ, IMRMOVQ, IOPQ, ICALL, 
		     IIADDQ,IPUSHQ, IRET, IPOPQ } : E_valB;
